...
const ContactList = React.memo(
  ({ conversations, setConversations, existingChat }) => {
    ...
    useEffect(() => {
      if (
        existingChat &&
        conversationRefs.current[existingChat.conversation_id]
      ) {
        conversationRefs.current[existingChat.conversation_id].scrollIntoView({
          behavior: "smooth",
          block: "center",
        });
      }
        ...
        {conversations.length > 0 ? (
          conversations.map((conversation, index) => (
            <div
              ...
              className="p-2 shadow-sm rounded-lg my-3 cursor-pointer"
              ref={(el) =>
                (conversationRefs.current[conversation.conversation_id] = el)
              }
          ...
          <div className="flex justify-center mt-4">
            <p className="text-center">
              No conversations found. Try searching for someone above, or
              browsing posts, blogs or activities.
            </p>
          </div>
        )}
      </div>
